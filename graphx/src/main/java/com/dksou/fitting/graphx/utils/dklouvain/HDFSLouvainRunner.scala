package com.dksou.fitting.graphx.utils.dklouvain

import com.dksou.fitting.graphx.utils.VertexState
...
//      graph.edges.saveAsTextFile(outputdir+"/level_"+level+"_edges")
      graph.vertices.map( {case (id,v) => ""+id+","+v.community }).saveAsTextFile(outputdir)
